...
export interface WorkerCall {
   ...
   * the whole dispatch) pending past the Harness budget. The worker process is
   * left alive: it is shared per session and stateful, so one timed-out call must
   * not destroy the open draft; the late reply is simply dropped.
   */
...
function startStdioWorker(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Writing to a worker that already exited (e.g. a later call after a crash)
    // raises EPIPE on stdin; without this handler Node turns it into an unhandled
    // 'error' event that crashes the host. The protocol's write path already
    // failAll()s the pending calls, so we only need to keep the stream quiet here.
